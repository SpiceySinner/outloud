...
import { asideResponseJsonSchema, asideResponseSchema } from "@/lib/aside-schema";
...
export async function POST(request: Request) {
  ...
  const { scene, focus, recentTurns, trigger, exchange } = parsed.data;
...
const asideSystemPrompt = [
  "You are the OutLoud coach. The learner has just stepped out of a Spanish roleplay to talk to you about something that is bothering them. You are on their side and you are not in character as anyone -- the person from the scene is not here.",
  "",
  "# How you talk",
  ...
  "Take what they say at face value. If they say the situation is useless to them, that is data, not resistance.",
  ...
  "",
  "2. Is the situation itself fine, but a DIFFERENT DIFFICULTY is the real one -- not where they are, but what breaks? Then kind=\"change_focus\". Set newFocus to the closest of these:",
  ...
  "   \"it is not the words, I just freeze\" is hesitation_pressure and is a change_focus, not a new scene. Someone naming a different difficulty in the same situation is asking you to look at a different thing, not to move them somewhere else.",
  "",
  "3. Otherwise kind=\"resume\", and this is the honest default. Most asides are someone thinking out loud. Going back to the same scene having been heard is a real outcome; inventing a change so the aside feels productive is not.",
  "   If they say they are ready to continue, that it is fine, or to keep going -- resume is the ONLY correct answer, and you offer it THAT TURN. Do not ask whether they are sure, do not ask another question, do not offer them something else. Someone who just said they want to carry on should not have to say it twice. Do not read tiredness or a bad day as a request to change the session.",
  "",
...
async function generateAsideTurn(input: AsideInput): Promise<AsideResponse> {
  ...
  return normalizeAsideTurn(generated, input.turnIndex);
}
 ...
 */
function normalizeAsideTurn(generated: AsideResponse, turnIndex: number): AsideResponse {
  let offer = turnIndex === 0 ? null : generated.offer;

  if (offer) {
    if (offer.kind === "change_scenario" && !(offer.newScenarioEn?.trim() && offer.newCharacter?.name.trim())) {
